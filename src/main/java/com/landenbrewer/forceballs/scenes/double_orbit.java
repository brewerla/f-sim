...
public class double_orbit extends Application {
	...
	public void start(Stage stage) {
		...
		boolean circle = true;
		double radius = 200;
		double satOrbitRadius = 173;

		final double[] goToLocation = {400.0, 400.0};

		Ball ball = new Ball(radius, 400, 800, 800, 0, 0, 5, Vector.MOON_MASS);
		ball.setVelocity(new Vector(Math.sqrt((Vector.BIG_G * Vector.EARTH_MASS) / (1000 * (goToLocation[1] - radius))) * 0.005, -90));
		...
		System.out.println(satToBall);

		satellite.setVelocity(new Vector(Math.sqrt((Vector.BIG_G * ball.getMass()) / (1000 * satToBall)) * 0.005, -90)
				.add(new Vector(Math.sqrt((Vector.BIG_G * Vector.EARTH_MASS) / (1000 * (goToLocation[1] - satOrbitRadius))) * 0.005, -90)));
		//satellite.setVelocity(new Vector(Math.sqrt((Vector.BIG_G * Vector.EARTH_MASS) / (1000 * (goToLocation[1] - 175))) * 0.005, -90));

		/*Circle path = new Circle(400, 400, 225);
		...
		Timeline animation = new Timeline(new KeyFrame(Duration.millis(5), e ->  {
			...
			double ballGravity = ball.getMass() * Vector.gravity(Vector.EARTH_MASS, (ballMagnitude * 1000)); //Starts at 200, total distance of 160000
			...
			double satGravity = satellite.getMass() * Vector.gravity(Vector.MOON_MASS, (satToBallMag * 1000));

			double satGravityOfCenter =  satellite.getMass() * Vector.gravity(Vector.EARTH_MASS, (satToCenterMag * 1000));
